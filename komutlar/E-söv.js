exports.run = async (client, msg, args) => {
  let kufur=[
    ...
    "Anasının amına rövaşata çekip molada sucuklu yumurta pişirdiğimin ucubesi"
  ]
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "698874337817788416")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 ...
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Birşeyler ters gitti @SiberFox#1112 etiketlemişsin yarram!`)
}})
  if(member.id === "669922126215774214")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Orda durucan Yapımcım @Gereksiz_Cause#0105'ya sövemezsin amını sikerim.`)
}})
